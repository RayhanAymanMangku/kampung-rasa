...
export const DineInTable = ({ orders, handleDeleteOrder }) => {
    const [showListPesanan, setShowListPesanan] = useState(false);
    const [selectedOrderId, setSelectedOrderId] = useState(null);

    const handleLihatClick = (idPesanan) => {
        setSelectedOrderId(idPesanan);
    ...
    return (
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                {orders.length > 0 ? orders.map(order => (
                    <tr key={order.idPesanan} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                            <button onClick={() => handleLihatClick(order.idPesanan)} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded font-bold text-xs">LIHAT</button>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                            <button onClick={() => handleDeleteOrder(order.idPesanan)} className="bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded font-bold text-xs">SELESAI</button>
                        </td>
                    </tr>
                )) : (
                    <tr>
            ...
            {showListPesanan && <DisplayMenuModal setShowListPesanan={setShowListPesanan} selectedOrderId={selectedOrderId} />}
        </table>
